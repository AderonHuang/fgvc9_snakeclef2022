...
def build_transform(is_train, config):
    ...
    if resize_im:
        if config.TEST.CROP:
            ...
            t.append(
                transforms.Resize(size,_pil_interp(config.DATA.INTERPOLATION)) #, interpolation=transforms.InterpolationMode.BICUBIC),
                # to maintain same ratio w.r.t. 224 images
            )
            t.append(transforms.CenterCrop(config.DATA.IMG_SIZE))
        else:
            t.append(
                transforms.Resize((config.DATA.IMG_SIZE, config.DATA.IMG_SIZE), _pil_interp(config.DATA.INTERPOLATION))#,
                                  #interpolation=transforms.InterpolationMode.BICUBIC)
            )
    # t.append(transforms.RandomHorizontalFlip(1.0))
    ...
    # t.append(transforms.Lambda(lambda crops: torch.stack([transforms.Normalize(IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)(crop) for crop in crops])))
