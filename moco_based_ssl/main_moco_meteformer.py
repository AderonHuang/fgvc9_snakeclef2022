...
def model_config(config_path):
    ...
    return config
from models import build_model
config_path = "./configs/MetaFG_2_384.yaml"
config = model_config(config_path)
metaformer = build_model(config)
# metaformer.train()
# metaformer.cuda()
...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        print(main_worker,ngpus_per_node)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        model = moco.builder.MoCo_ViT(
            ...
            args.moco_dim, args.moco_mlp_dim, args.moco_t,args.arch)
    
    elif args.arch=="meta":
        model = moco.builder.Meta(metaformer,
            args.moco_dim, args.moco_mlp_dim, args.moco_t,args.arch)
#         model.train()
#         model.cuda()
    else:
...
#         model, optimizer = amp.initialize(model, optimizer, opt_level="O1")
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            args.start_epoch = 0 if "epoch" not in  checkpoint.keys() else checkpoint['epoch'] 
    ...
    # Data loading code
    traindir = os.path.join(args.data,'ssl_dataset')
    #traindir = args.data
    ...
    augmentation1 = [
        transforms.RandomResizedCrop(384, scale=(args.crop_min, 1.)),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename=os.path.join(args.save_dir,'checkpoint_%04d.pth.tar' % epoch))
