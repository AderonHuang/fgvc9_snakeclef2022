...
def create_dataloaderval(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      local_rank=-1, world_size=1):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache.
    with torch_distributed_zero_first(local_rank):
...
#---------------------
class LoadImagesWithoutLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            img_path = "/home/jovyan/xugan/datasets/snakeclef2022_test/SnakeCLEF2022-large_size/"
            dicts = {}
            ff = []
            ...
            for img_name in img_names:
                ...
                dicts[tmp] = img_name
                ff.append(os.path.join(img_path,img_name))
            self.img_files = sorted(
                [x.replace('/', os.sep) for x in ff if os.path.splitext(x)[-1].lower() in img_formats])
...
#             assert not augment, '%s. Can not train without labels.' % s
        ...
        if cache_images:
            ...
            for i in pbar:  # max 10k images
                self.imgs[i],self.img_hw0[i], self.img_hw[i] = load_image(self, i)  # img, img0,hw_original, hw_resized
...
#         return x
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            # Load image
            img0,img00, (h0, w0), (h, w) = load_image(self, index)

            # Letterbox
            #shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img0, new_shape=self.img_size, auto=False, scaleup=self.augment)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            ff = []
            for x in f:
                img_num = x.split("/")[-1].split(".")[0]
                ff.append(os.path.join(img_path,dicts[img_num]))
...
def load_image1(self, index):
    ...
    if img is None:  # not cached
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR
            img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp)
        return img,img0, (h0, w0), img.shape[:2]  # img, hw_original, hw_resized
    else:
        return self.imgs[index], self.img_hw0[index], self.img_hw[index]  # img, hw_original, hw_resized
