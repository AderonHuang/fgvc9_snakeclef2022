...
def find_images_and_targets_snakeclef2022_trainvalall(root,dataset,istrain=False,aux_info=False,over_sampling=False):
    ...
    if istrain:
        data_file_list = [
            os.path.join(root,'train_split.csv'),
            os.path.join(root,'valid_split.csv')
        ]
        items_all = []
...
def find_images_and_targets_snakeclef2022_all(root,dataset,istrain=False,aux_info=False,over_sampling=False):
    ...
    num_ref = sorted_hist[idx_ref]
    imgs_root = os.path.join(root, 'SnakeCLEF2022-small_size', 'SnakeCLEF2022-small_size')
    imgs_test = os.path.join(root, 'SnakeCLEF2022-test_images', 'SnakeCLEF2022-large_size')
    test_file = os.path.join(root, 'SnakeCLEF2022-TestMetadata.csv')
    ...
    if istrain:
        ...
        items_all = []
        for data_file in data_file_list:
            items_tmp = load_csv_annotations(fp=data_file, row_length=5)
            items_all.extend(items_tmp)
            # print('ddd', data_file, len(items_tmp))
    else:
        items_all = load_csv_annotations(fp=os.path.join(root,'valid_split.csv'))

    # print('items_all', len(items_all))
    images_and_targets = []
    images_info = []
    for it in items_all:
        if '/' in it['file_path']:
            ...
            file_path = os.path.join(imgs_test, it['file_path'])
        if 'class_id' not in it:
            target = -1 #1000000+i
            num_cur = 1
        else:
            class_name = it['class_id']
...
class DatasetMeta(data.Dataset):
    def __init__(
        ...
        elif dataset == 'snakeclef2022all':
            images,class_to_idx,images_info = find_images_and_targets_snakeclef2022_all(root,dataset,train,aux_info,over_sampling)
