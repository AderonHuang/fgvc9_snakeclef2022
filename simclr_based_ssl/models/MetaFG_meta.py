...
#             assert len(meta_dims)==extra_token_num-1
        ...
        # ArcFace
        # self.arc_face = ArcFace(in_features=num_classes, out_features=num_classes)

        # amsoftmax 
        # self.head = AMSoftmax(attn_embed_dims[-1], num_classes)
    ...
    def forward(self, x,meta=None):
        if meta is not None:
            ...
            if cur_mask_prob != 0 and self.training:
                ...
                mask[mask_index] = 0
                meta = mask * meta
        feat = self.forward_features(x,meta)
        x = self.head(feat)
        # x = self.arc_face(x)
        return feat, x
